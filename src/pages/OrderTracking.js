...
const OrderTracking = () => {
  ...
  // Function to calculate time remaining - 24 jam sejak order dibuat
  const calculateTimeRemaining = useCallback((orderData) => {
    ...
    const timeLeft = twentyFourHoursInMs - timeElapsed;
  ...
  }, [order, tokenExpired, timeRemaining]);

  // Initial fetch when URL params are present
  useEffect(() => {
      ...
      const initialFetch = async () => {
        ...
        try {
          ...
          if (error.response?.status === 404) {
            ...
            setError('Format nomor pesanan atau token tidak valid. Silakan periksa kembali.');
          } else if (error.response?.status >= 500) {
            setError('Server sedang mengalami gangguan. Silakan coba lagi dalam beberapa saat.');
          } else {
            setError('Terjadi kesalahan saat memuat pesanan. Silakan coba lagi.');
          }
        } finally {
          setLoading(false);
        }
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        {order && !tokenExpired && (
          <div className="space-y-6">
            {/* Token Expiry Warning - Show for all orders */}
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3 flex-1">
                    <p className="text-sm text-yellow-700">
                      <span className="font-semibold">Token akan kedaluwarsa dalam:</span>{' '}
            ...
            <div className="bg-white rounded-xl shadow-md p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600 font-medium">Manual Refresh</span>
                  <span className="text-xs text-gray-500">• Klik tombol refresh untuk update status</span>
                </div>
